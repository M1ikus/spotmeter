...
Wszystkie ścieżki są względne do **roota repo** (folder z `tools/`, nie `mods/spotmeter/`). Patrz [`../../README.md`](../../README.md) — top-level overview monorepo.
...
### Kompilacja .pyc (z roota repo)

```sh
"C:/Users/23120/miniforge3/envs/py27/python.exe" -c "import py_compile; py_compile.compile('mods/spotmeter/src/mod_spotmeter.py', cfile='mods/spotmeter/build/mod_spotmeter.pyc', doraise=True)"
```

### Pakowanie .wotmod (release)

```sh
py -3.10 tools/build_wotmod.py spotmeter
```

Output do `mods/spotmeter/dist/`:
- `spotmeter-v<wersja>.wotmod` — sam mod (do `<WoT>/mods/2.2.1.2/`)
- `spotmeter.json` — domyślny config (do `<WoT>/mods/configs/`)
- `INSTALL.txt` — instrukcja
- `spotmeter-v<wersja>.zip` — wszystko w jednym do dystrybucji

Wersja jest czytana z `mods/spotmeter/meta.xml` — zaktualizuj tam przed kolejnym buildem.

### Hot-test podczas devu

```sh
cp mods/spotmeter/build/mod_spotmeter.pyc "D:/Gry/World_of_Tanks_EU/res_mods/2.2.1.2/scripts/client/gui/mods/"
cp mods/spotmeter/src/spotmeter.json "D:/Gry/World_of_Tanks_EU/mods/configs/"
```
